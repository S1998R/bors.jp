window.addEventListener("load", function(event) {
    ...
    for (let i = 1; i <= 9; i++) {
        ...
        var userimage = "user-image_" + i;

        if (i == 1) {
            userimage = "user-image"
        }

        var imagecancelicon = "image_cancel_icon_" + (i+1);
        var cameraicon = "camera_icon_" + (i+1);
        var imagespan = "#image_span_" + (i+1);
        var input = "<input type='file' name='image_" + (i+1) + "'" + " accept='image/*' id='id_image_" + (i+1) + "'" + " class='js-upload-file' onchange='previewImage_" + (i+1) + "(this);'>";
        document.getElementById(cameraicon).classList.add("display");
        document.getElementById(imagecancelicon).classList.add("display");
        $(imagespan).after(input);
...
function previewImage_10(obj)
{
  ...
  $('#edit_submit_button').click(function(){
    var default_image = document.getElementById("default_image").title;
    var default_image_2 = document.getElementById("default_image_2").title;
    var default_image_3 = document.getElementById("default_image_3").title;
    var default_image_4 = document.getElementById("default_image_4").title;
    var default_image_5 = document.getElementById("default_image_5").title;
    var default_image_6 = document.getElementById("default_image_6").title;
    var default_image_7 = document.getElementById("default_image_7").title;
    var default_image_8 = document.getElementById("default_image_8").title;
    var default_image_9 = document.getElementById("default_image_9").title;
    var default_image_10 = document.getElementById("default_image_10").title;
    ...
    if (!image && document.getElementById("image_clear").value){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>商品のトップ画像は必須です。";
        return false;
    }
    if (!merchandise_name){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>商品名は必須です。";
        return false;
    }
    if (!value){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>価格は必須です。";
        return false;
    }
    if (!merchandise_status){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>商品の状態は必須です。";
        return false;
    }
    if (!category){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>カテゴリは必須です。";
        return false;
    }
    if (!region){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>商品の受け渡しが可能なエリアは必須です。";
        return false;
    }
    if (!explanation){
        document.getElementById("edit_error").innerHTML = "<i class='fas fa-exclamation-circle'></i>商品説明は必須です。";
        return false;
    }

    document.getElementById("edit_error").innerHTML = "<i class='fas fa-paper-plane'></i> 送信中...";

    var button = document.getElementById("edit_submit_button");
    button.disabled = true

    var sell_form = document.getElementById('edit_form');
    ...
    if(blob) {
        ...
        document.getElementById("edit_error").innerText = '画像1はアップロードできません。';
    }
    try {
    if(blob_2) {
        ...
        document.getElementById("edit_error").innerText = '画像2はアップロードできません。';
    }
    try {
    if(blob_3) {
        ...
        document.getElementById("edit_error").innerText = '画像3はアップロードできません。';
    }
    try {
    if(blob_4) {
        ...
        document.getElementById("edit_error").innerText = '画像4はアップロードできません。';
    }
    try {
    if(blob_5) {
        ...
        document.getElementById("edit_error").innerText = '画像5はアップロードできません。';
    }
    try {
    if(blob_6) {
        ...
        document.getElementById("edit_error").innerText = '画像6はアップロードできません。';
    }
    try {
    if(blob_7) {
        ...
        document.getElementById("edit_error").innerText = '画像7はアップロードできません。';
    }
    try {
    if(blob_8) {
        ...
        document.getElementById("edit_error").innerText = '画像8はアップロードできません。';
    }
    try {
    if(blob_9) {
        ...
        document.getElementById("edit_error").innerText = '画像9はアップロードできません。';
    }
    try {
    if(blob_10) {
        ...
        document.getElementById("edit_error").innerText = '画像10はアップロードできません。';
    }

    $.ajax({
      url: "http://127.0.0.1:8000/merchandise_edit_post/",
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false,
    ...
    .done((response => {
        if (response.message) {
             document.getElementById("edit_error").innerText = response.message;
             return;
            }
            merchandise_pk = document.getElementById("merchandise_pk").title;
            location.href = "http://127.0.0.1:8000/merchandise_detail/" + merchandise_pk + "/?status=success";

        }))
    .fail(function( jqXHR, textStatus, errorThrown ) {
      button.disabled = false
      document.getElementById("edit_error").innerText = '送信に失敗しました。';
    });
...
function FacultyChangeSelection(){
    ...
    if (text=='追加する') {
        ...
        $('#id_department').append($('<option>').html("学部を選択して下さい").val(""));
    ...
    $.ajax({
        ...
        .done((response => {
              ...
              var count=1;
              var department_list_len = department_list.length
              for( var department in department_list ) {
                        if (count==department_list_len){
                            $('#id_department').append($('<option>').html(department_list[department]).val(""));
                            count++;
                        }else {
